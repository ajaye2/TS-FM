...
def model_factory(config, feat_dim, labels_df=None, class_names=None):
    task = config['task']
    ...
    if max_seq_len is None:
        raise ValueError("Maximum sequence length must be defined with the script argument `max_seq_len`")

    # print("Model factory: task = {}, feat_dim = {}, max_seq_len = {}".format(task, feat_dim, max_seq_len))
    ...
    if (task == "classification") or (task == "regression"):
        num_labels = len(class_names) if task == "classification" else labels_df.shape[1]  # dimensionality of labels
        if config['model'] == 'LINEAR':
