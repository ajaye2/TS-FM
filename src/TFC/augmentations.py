...
def DataTransform_FD(sample, config):
    """Weak and strong augmentations in Frequency domain """
    aug_1 = remove_frequency(sample, pertub_ratio=0.1, config=config)
    aug_2 = add_frequency(sample, pertub_ratio=0.1, config=config)
    aug_F = aug_1 + aug_2
    return aug_F

def remove_frequency(x, pertub_ratio=0.0, config=None):
...
def add_frequency(x, pertub_ratio=0.0, config=None):
